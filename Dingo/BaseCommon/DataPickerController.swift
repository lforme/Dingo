...
typealias PickerResult = (component: Int, value: String)
            ...
            let dataPicker = DataPickerController(title: title, data: items)
                .then { vc in
                    ...
                    vc.confirmButton.rx.action = CocoaAction { [unowned vc] in
                        ...
                        switch items.count {
                        case 1:
                            result.append((0, items[0][vc.pickView.selectedRow(inComponent: 0)]))
                            observer.onNext(result)
                        case 2:
                            result.append((0, items[0][vc.pickView.selectedRow(inComponent: 0)]))
                            result.append((1, items[1][vc.pickView.selectedRow(inComponent: 1)]))
                            observer.onNext(result)
                        case 3:
                            result.append((0, items[0][vc.pickView.selectedRow(inComponent: 0)]))
                            result.append((1, items[1][vc.pickView.selectedRow(inComponent: 1)]))
                            result.append((2, items[2][vc.pickView.selectedRow(inComponent: 2)]))
                            observer.onNext(result)
